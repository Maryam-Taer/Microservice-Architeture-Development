...
def get_restaurant_records(s_url):
    try:
        ...
        response = requests.get(f'{s_url}finding-restaurant?timestamp={read_from_json()["last_updated"]}',
                                headers=headers)
        json_data = json.loads(response.text)
        logger.info(
            f'Query for restaurant search records after {read_from_json()["last_updated"]} returns {len(json_data)} '
            f'results.')
...
def get_review_records(s_url):
    try:
        ...
        json_data = json.loads(response.text)
        logger.info(
            f'Query for review records after {read_from_json()["last_updated"]} returns {len(json_data)} results.')
...
def calc_stat(restr_data, review_data):
    """ Calculate stats for restaurant searched records filtered by timestamp """
    takeout_counter = 0
    top_review_counter = 0

    all_rest_records = len(restr_data)
    all_review_records = len(review_data)
